...
// New user Details validating...
    const signUpvalid = ()=>{
        if(Username.length<4){
            setNameError({nameError:"your name must be 5 letters",redError:Boolean(true)})
            return false;
        }
        ...
        if(Password.length<8){
            setPassowordError({passwordError:'please enter strong password (minimum 8 letters)',Perror:Boolean(true)})
            return false;
        }
